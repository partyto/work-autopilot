...
/**
 * Slack 메시지 전체 본문을 30자 이내 할일 제목으로 요약.
 * API Key 없거나 실패 시 앞 30자로 fallback.
 */
export async function summarizeSlackTitle(text: string): Promise<string> {
  const ai = getClient();
  if (!ai) return text.substring(0, 30);
  ...
  try {
    const msg = await ai.messages.create({
      ...
      messages: [{
        role: "user",
        content: `다음 Slack 메시지를 할일 제목으로 30자 이내 한국어로 요약해. 동사형으로 끝내. 따옴표 없이 제목만 출력:\n${text}`,
      }],
    });
