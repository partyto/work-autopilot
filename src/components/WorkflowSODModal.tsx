...
import { X, Plus, Loader2, AlertCircle, Calendar, Flag } from "lucide-react";
...
interface SODPreview {
  carriedOver: TaskItem[];
  dueToday: TaskItem[];
  overdueNow: Array<{ id: string; title: string; priority: string; dueDate?: string | null }>;
...
export default function WorkflowSODModal({ onClose, onSent }: Props) {
  const [preview, setPreview] = useState<SODPreview | null>(null);
  ...
  useEffect(() => {
    fetch("/api/daily/preview?type=sod")
      .then((r) => r.json())
      .then((d) => setPreview(d))
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success("하루 시작 리포트가 Slack으로 전송됐어요");
        onSent();
      } else {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div>
            <h2 className="text-sm font-semibold text-slate-900 tracking-tight">🌅 하루 시작</h2>
            {preview && (
              <p className="text-[11px] text-slate-400 mt-0.5">{preview.todayStr} · 진행 중 {preview.inProgressCount}건 · 대기 {preview.pendingCount}건</p>
            )}
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          {loading ? (
            ...
            <>
              {/* 어제에서 이관 */}
              <Section
                title={`📥 어제에서 이관 (${preview.carriedOver.length}건)`}
                empty={preview.carriedOver.length === 0}
                emptyText="없음"
              >
                {preview.carriedOver.map((t) => (
                  <TaskRow key={t.id} title={t.title} priority={t.priority} badge={STATUS_LABEL[t.status]} />
                ))}
              </Section>

              {/* 오늘 마감 */}
              {preview.dueToday.length > 0 && (
                <Section title={`📅 오늘 마감 (${preview.dueToday.length}건)`}>
                  {preview.dueToday.map((t) => (
                    <TaskRow key={t.id} title={t.title} priority={t.priority} badge="D-0" badgeRed />
                  ))}
                </Section>
              )}

              {/* 기한 초과 */}
              {preview.overdueNow.length > 0 && (
                <Section title={`⚠️ 기한 초과 (${preview.overdueNow.length}건)`}>
                  {preview.overdueNow.map((t) => (
                    <TaskRow key={t.id} title={t.title} priority={t.priority} badge={t.dueDate?.slice(0, 10)} badgeRed />
                  ))}
                </Section>
              )}
              ...
              {/* 이미 추가한 태스크 */}
              {addedTasks.length > 0 && (
                <Section title={`✅ 방금 추가한 할일 (${addedTasks.length}건)`}>
                  {addedTasks.map((t, i) => (
                    <TaskRow key={i} title={t.title} priority={t.priority} />
                  ))}
                </Section>
              )}
...
function TaskRow({
  ...
  return (
    <div className="flex items-center gap-2 py-1 px-1">
      <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${PRIORITY_DOT[priority] || "bg-slate-300"}`} />
      <span className="text-xs text-slate-700 flex-1 truncate" style={{ wordBreak: "keep-all" }}>{title}</span>
      {badge && (
        <span className={`text-[10px] px-1.5 py-0.5 rounded-md font-medium flex-shrink-0 ${
          badgeRed ? "bg-red-50 text-red-600 border border-red-200" : "bg-slate-100 text-slate-500"
        }`}>
          {badge}
        </span>
      )}
